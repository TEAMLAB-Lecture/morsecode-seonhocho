...
def is_validated_english_sentence(user_input):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    def has_number() -> bool:
        return any(s.isnumeric() for s in user_input)

    def has_special_charater() -> bool:
        return any(s in '_@#$%^&*()-+=[]{}\"\';:\\|`~' for s in user_input)

    def is_empty() -> bool:
        return len([s for s in user_input if s not in '.,!? ']) == 0

    return not has_number() and not has_special_charater() and not is_empty()
    # ==================================
...
def is_validated_morse_code(user_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    def has_valid_charater() -> bool:
        return all(c in '-., ' for c in user_input)

    def has_valid_code() -> bool:
        morse_codes = set(morse for _, morse in get_morse_code_dict().items())
        return all(word in morse_codes for word in user_input.split())

    return has_valid_charater() and has_valid_code()
    # ==================================
...
def get_cleaned_english_sentence(raw_english_sentence):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    return ''.join(
        s for s in raw_english_sentence if s not in '.,!?').strip()
    # ==================================
...
def decoding_character(morse_character):
    ...
    result = None
    for alphabet, morse in morse_code_dict.items():
...
def decoding_sentence(morse_sentence):
    ...
    def decode_word(word: str) -> str:
        return decoding_character(word) if word else ' '

    return ''.join(decode_word(word) for word in morse_sentence.split(' '))
    # ==================================
...
def encoding_sentence(english_sentence):
    ...
    def encode_word(word: str) -> str:
        return ' '.join(encoding_character(c) for c in word)

    clean_sentence = get_cleaned_english_sentence(english_sentence)
    return '  '.join(encode_word(word) for word in clean_sentence.split())
    # ==================================
